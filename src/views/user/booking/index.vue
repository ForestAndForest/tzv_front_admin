  ...
  <CommonPage>
    ...
    >
      ...
      <MeQueryItem :label-width="50" label="邮箱">
        <n-input v-model:value="queryItems.email" clearable placeholder="请输入邮箱" type="text" />
      </MeQueryItem>
      ...
      <MeQueryItem :label-width="70" label="用户类型">
        <n-select v-model:value="queryItems.type" :options="userType" clearable />
      </MeQueryItem>
    </MeCrud>
    ...
    <MeModal
      ...
      width="520px"
    >
      <n-form :label-width="80" label-align="left" label-placement="left">
        ...
        <n-form-item label="性别">
          <n-text v-if="modalForm.gender">
            {{ genders.find((item) => item.value === modalForm.gender).label }}
          </n-text>
        ...
        <n-form-item label="用户类型">
          <n-tag :type="modalForm.type !== 0 ? 'error' : 'default'" clearable>
            {{ userType.find((item) => item.value === modalForm.type).label }}
          </n-tag>
        </n-form-item>
...
const columns = [
  ...
  { title: '用户名', key: 'username', width: 150, ellipsis: { tooltip: true } },
  {
    ...
    render: (row) =>
      h(
        NTag,
        { type: row.type !== 0 ? 'error' : 'default' },
        () => userType.find((item) => item.value === row.type).label
      ),
  },
  {
    title: '性别',
...
const { modalRef, modalFormRef, modalForm, modalAction, handleAdd, handleOpen, handleSave } =
  useCrud({
    name: '用户',
